...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Vinchy Zapas V20", layout="wide", page_icon="⚡")
...
def cargar_datos():
    ...
    if sheet:
        try:
            ...
            # Si hay datos, creamos el DF. Si está vacío (lista vacía), devolvemos DF con columnas
            if data:
                ...
                return df[cols], True # True = Conexión Éxito
            else:
                # Si la hoja está vacía de verdad (porque es nueva), devolvemos vacío pero con éxito
                return pd.DataFrame(columns=cols), True
        except: 
            pass
    
    # Si llegamos aquí es que falló la conexión. Devolvemos False
...
# CARGAMOS DATOS Y VERIFICAMOS CONEXIÓN
df, conexion_ok = cargar_datos()

if not conexion_ok:
    st.error("⚠️ ERROR CRÍTICO DE CONEXIÓN CON GOOGLE SHEETS")
    st.warning("No se han podido cargar los datos. El sistema se ha bloqueado para evitar sobrescribir tu inventario.")
    st.stop() # DETENEMOS EL PROGRAMA AQUÍ PARA QUE NO PUEDAS GUARDAR NADA
...
if op == "👟 Nuevo Producto":
    ...
    with st.form("fc"):
        ...
        if marca_sel == "Otras": marca_final = c1.text_input("Escribe la Marca", key="k_marca_otro")
        ...
        if tienda_sel == "Otras": tienda_final = c3.text_input("Escribe la Tienda", key="k_tienda_otro")
