...
st.set_page_config(page_title="Vinchy Zapas V72", layout="wide", page_icon="👟")
...
# --- 🧠 CAPA 2: LÓGICA (CONVERSIONES) ---
def texto_a_float(valor):
    """Convierte '45,50' (texto) a 45.5 (numero) SOLO para calcular."""
    if pd.isna(valor) or str(valor).strip() == "": return 0.0
    try:
        # Quitamos símbolo € y puntos de miles si los hubiera
        limpio = str(valor).replace("€", "").strip()
        if "." in limpio and "," in limpio: # Caso 1.200,50
...
def float_a_texto(numero):
    """Convierte 45.5 (numero) a '45,50' (texto) para guardar/mostrar."""
    try:
        return f"{float(numero):.2f}".replace(".", ",")
    except: return "0,00"

def arreglar_talla(valor):
    """Talla siempre es texto limpio"""
    v = str(valor).replace(".0", "").replace(",", ".").strip()
    if v == "nan": return ""
    return v

# --- 🧱 CAPA 1: ALMACENAMIENTO (GOOGLE SHEETS) ---
def conectar_sheets():
    try:
...
def obtener_listas(df):
    ...
    m_exist = df['Marca'].unique().tolist() if not df.empty else []
    t_exist = df['Tienda Origen'].unique().tolist() if not df.empty else []
    return sorted(list(set(BASES_MARCAS + m_exist))), sorted(list(set(BASES_TIENDAS + t_exist)))
...
# --- NUEVO ---
    ...
    with st.form("f_new"):
        ...
        pr = c5.text_input("Precio Compra (€)", placeholder="45,50")
        ...
        if st.form_submit_button("GUARDAR EN STOCK"):
            if not mod or not mf: st.error("Falta Marca/Modelo")
                ...
                new = {
                    ...
                    "Plataforma Venta":"", "Cuenta Venta":"", "Precio Compra":p_str, "Precio Venta":"0,00", 
                    "Estado":"En Stock", "Ganancia Neta":"0,00", "ROI %":"0,00", "Tracking":""
                }
    ...
    if sel != "-":
        ...
        with st.form("fv"):
            ...
            if st.form_submit_button("CONFIRMAR"):
                ...
                df.at[idx,'Precio Venta'] = float_a_texto(pv_float) # CORREGIDO AQUI
                df.at[idx,'Plataforma Venta'] = pl
                df.at[idx,'Cuenta Venta'] = cu
                df.at[idx,'Ganancia Neta'] = float_a_texto(gan) # CORREGIDO AQUI
...
# --- HISTORIAL ---
    ...
    df_ed = st.data_editor(df_v[[c for c in cols_ord if c in df_v.columns]], column_config=col_cfg, hide_index=True, use_container_width=True, key="ev72")
    ...
    if st.button("💾 GUARDAR CAMBIOS EN LA NUBE", type="primary"):
        for i, row in df_ed.iterrows():
            ...
            if real_idx:
                ...
                df.at[idx, 'Ganancia Neta'] = float_a_texto(pv - pc) # CORREGIDO AQUI
